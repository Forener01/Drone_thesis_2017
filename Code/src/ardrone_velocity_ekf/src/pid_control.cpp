...
PID_Control::PID_Control() {
  ...
  params.param<std::string>("odometry_topic", s2, "ardrone/odometry");
  ...
  // subscribe
  cmd_sub = nh_.subscribe(s1, 1, &PID_Control::InputCallback, this,
                          ros::TransportHints().tcpNoDelay());
  odo_sub = nh_.subscribe(s2, 1, &PID_Control::OdoCallback, this,
                          ros::TransportHints().tcpNoDelay());
  ping_sub = nh_.subscribe(s4, 1, &PID_Control::PingCallback, this);
  cam_sub = nh_.subscribe(s5, 1, &PID_Control::CamCallback, this,
                          ros::TransportHints().tcpNoDelay());

  // Publish
  cmd_pub = nh_.advertise<geometry_msgs::Twist>(s3, 1);
  error_pub =
      nh_.advertise<geometry_msgs::Twist>("tud_ekf/vel_error_topic", 100);
  percent_error_pub = nh_.advertise<geometry_msgs::Twist>(
      "tud_ekf/vel_percent_error_topic", 100);

  // Dynamic parameter reconfigure
  dynamic_reconfigure::Server<
      ardrone_velocity_ekf::dynamic_param_configConfig>::CallbackType f;
...
void PID_Control::CamCallback(const geometry_msgs::PoseStamped &cam) {
  ...
  tf::Quaternion world(cam.pose.orientation.x, cam.pose.orientation.y,
                       cam.pose.orientation.z, cam.pose.orientation.w);
...
void PID_Control::dynamic_reconfigure_callback(
    ardrone_velocity_ekf::dynamic_param_configConfig &config, uint32_t level) {
  //    if (first_reconfig)
  //    {
  //      first_reconfig = false;
  //      return;     // Ignore the first call to reconfigure which happens at
  //      startup
  //    }
  gain_xy(0) = config.Kp;
  gain_xy(1) = config.Ki;
  ...
  ROS_INFO("Pid reconfigure request: Kp: %f, Ki: %f, Kd: %f ", gain_xy(0),
           gain_xy(1), gain_xy(2));
...
void PID_Control::pid_control() {
  ...
  derivative_error(0) =
      derv_filter * error_derivative(0) + derv_smith * smith_x;
  derivative_error(1) =
      derv_filter * error_derivative(1) + derv_smith * smith_y;

  Eigen::Vector2d derivative_vel;
  derivative_vel(0) =
      -derv_filter * error_vel_derivative(0) - derv_smith * smith_vx;
  derivative_vel(1) =
      -derv_filter * error_vel_derivative(1) - derv_smith * smith_vy;

  Control(2, 0) =
      std::min(max(0), std::max(-max(0), vl * derivative_vel(0)) +
                           er * derivative_error(0)); // D-Term x-direction
  Control(2, 1) =
      std::min(max(1), std::max(-max(1), vl * derivative_vel(1)) +
                           er * derivative_error(1)); // D-Term y-direction

  // Intergral part with max values and reset if new reference value is received
  // -> I-Term
  i_term_set(i_term(0), error_xy(0), command_vec(0), old_command_vec(0),
             wind_up(0), dt.toSec());
  i_term_set(i_term(1), error_xy(1), command_vec(1), old_command_vec(1),
             wind_up(1), dt.toSec());
  ...
  control_output_pid.linear.x = Control(0, 0) * gain_xy(0) +
                                Control(1, 0) * gain_xy(1) +
                                Control(2, 0) * gain_xy(2);
  ...
  control_output_pid.linear.x = std::min(
      max_output(0), std::max(-max_output(0), control_output_pid.linear.x));
  control_output_pid.linear.y = std::min(
      max_output(1), std::max(-max_output(1), control_output_pid.linear.y));
  ...
  //        ROS_INFO("Vel   : %f , %f", odo.twist.twist.linear.x,
  //        odo.twist.twist.linear.y);
  //        ROS_INFO("Error : %f ,  %f", error_xy(0),error_xy(1));
  //        ROS_INFO("pterm | iterm | dterm   : %f | %f | %f", tmp(0), tmp(1),
  //        tmp(2));
  //        ROS_INFO("Cmd   : %f , %f", control_output_pid.linear.x,
  //        control_output_pid.linear.y);
  //        ROS_INFO("Ref Yaw: %f,  Yaw: %f,  DYaw: %f,  YawRate: %f",
  //        command.angular.z*180/M_PI,
  //        euler(2)*180/M_PI,yaw_error,yaw_error*gain_yaw);
  //    }
...
void PID_Control::i_term_set(double &i_value, double error, double vel,
                             double old_vel, double wind_up, double dt) {
...
void PID_Control::run() { ros::spinOnce(); }
